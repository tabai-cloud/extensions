(function(){function e(e){return e}var t=`data-tabai-ownership-processed`,n=/\/chat\/([^/?#]+)/;function r(e){let t=e?.getAttribute(`href`);return t?n.exec(t)?.[1]??null:null}function i(e){return!e||!e.startsWith(`chat:`)?null:e.slice(5)}function a(e){let n=[],a=e.querySelectorAll(`[data-row]:not([${t}])`);for(let e of a){let a=e.querySelector(`[data-row-main-button]`);if(!a)continue;let o=i(e.closest(`[data-row-key]`)?.getAttribute(`data-row-key`)??null)??r(a),s=e.querySelector(`[data-row-action]`);!o||!s||(e.setAttribute(t,`1`),n.push({resourceId:o,mount:e=>s.before(e)}))}return n}function o(e){let n=[],i=e.querySelectorAll(`table[data-cds="Table"] tr[data-hoverable]:not([${t}])`);for(let e of i){let i=e.querySelector(`a[href*="/chat/"]`),a=r(i),o=e.querySelector(`button[aria-haspopup="menu"]`);if(!a||!o)continue;let s=i?.nextElementSibling?.firstElementChild,c=s instanceof HTMLElement?e=>s.append(e):e=>o.before(e);e.setAttribute(t,`1`),n.push({resourceId:a,mount:c})}return n}var s=`Solicitar acesso`,c=`Erro — tentar novamente`,l=`Acesso concedido`,u=`data-tabai-request-ownership`,d=`data-tabai-ownership-badge`,f=`data-tabai-force-label`,p=`tabai-icon`,m=`tabai-label`,h=`tabai-ownership-styles`;function g(){if(document.getElementById(h))return;let e=document.createElement(`style`);e.id=h,e.textContent=`
    [${u}], [${d}] {
      all: unset;
