...
Separate ai-cloud-operator workloads — each with its own user — can share
one underlying claude.ai account. Anthropic's own API only knows about
"account", not "workload": nothing stops one workload's browser from
fetching another workload's conversation just by addressing it directly
(a guessed or observed conversation id), which would leak one user's chat
history to a completely different user sharing the same account. This
addon enforces per-workload chat ownership at the proxy layer so that
never happens, regardless of what the webapp or sidebar UI does or doesn't
show:

- **Ownership set.** `_owned_chat_ids` is this workload's own view of which
  conversation ids belong to it. It is hydrated from — and only from — this
  workload's own operator, never Convex directly, the same rule every other
  operator interaction in this addon already follows:
  - **Bootstrap**: a blocking `GET {operatorApiBaseUrl}/workloads/
    {workloadName}/integrations/ownership?source=claude&type=chat` at
    module load, before `mitmdump` serves a single flow — source/type are
    this addon's own constants (`INTEGRATION_SOURCE`/`INTEGRATION_TYPE`),
    sent explicitly rather than assumed by the operator or Convex, since
    that route/table is deliberately generic across resource kinds (a
    Claude "Project" page is a concrete near-term second case). Not (yet)
    enrolled or the fetch fails outright: logs and
    falls through with an empty set rather than blocking forever — same
    best-effort posture as everything else in this file. Every failure mode
    (including anything `urlopen` can raise, not just `URLError`) is caught
    here specifically because an uncaught exception at import time kills
    `mitmdump` itself, not just this one feature.
  - **New-conversation detection**: the webapp's `POST .../completion`
    request body carries a `create_conversation_params` field on — and only
    on — the very first message of a brand-new conversation. When seen, the
    conversation id (parsed out of the URL) is added to `_owned_chat_ids`
    immediately (optimistic, local, so the very next request — e.g. the
    browser re-fetching the conversation it just created — doesn't
    false-negative before a round trip completes) and registered with the
    operator via a best-effort `POST .../integrations/ownership` (body
    `{"source": "claude", "type": "chat", "resourceId": "<conversation id>"}`).
    This detection is webapp-only: Claude for Chrome's sidebar sends
    (`api.anthropic.com/v1/messages`) carry no conversation id anywhere in
    the URL and, confirmed against a real captured session, never include a
    `create_conversation_params` field either — a sidebar-originated new
    chat is a real, known coverage gap here, not a silent omission.
  - **Poll refresh**: a daemon thread re-runs the same fetch on an interval,
    **jittered** rather than fixed (`random.uniform(45, 75)` seconds each
    tick, not a flat 60) — the same reasoning behind OPA's own bundle-
    polling `polling.min_delay_seconds` / `polling.max_delay_seconds`
    config fields (see [OPA's
    docs](https://www.openpolicyagent.org/docs/)): every sidecar in a fleet
    that imported this module within the same second (a Deployment
    rollout, a mass node restart) would otherwise poll the operator in
    lockstep forever, with nothing here ever resyncing their phase apart. A
    successful fetch **replaces**
    `_owned_chat_ids` wholesale, not merges — a chat revoked upstream must
    disappear locally too. A *failed* fetch changes nothing, deliberately:
    a single dropped request must never lock a user out of their own chat
    history.
- **Enforcement**: any `GET` to `.../chat_conversations/{id}` — a read —
  whose `{id}` is not in `_owned_chat_ids` gets hard-blocked with a
  synthesized `403` before the request ever reaches claude.ai. The `POST
  .../completion` / `.../retry_completion` endpoints share this same URL
  shape but are sends, not reads, and are handled by the detection logic
  above instead.
- **Deliberately NOT done yet**: filtering `.../chat_conversations_v2` (the
  account-wide chat list) down to owned entries only. We don't yet know the
  real endpoint the webapp's chat list actually renders from — live
  investigation so far suggests it's bundled into some bootstrap payload,
  not a dedicated list call — and guessing at the wrong endpoint or an
  unconfirmed response shape would be worse than leaving it alone. This is
  a deferred follow-up, not an oversight; same goes for `.../chat_conversations/
  {id}/title` (a rename) and `.../artifacts/{id}/versions` (a different
  resource, keyed by its own id) — both are also technically account-scoped
